...
class CurriculumRoute:
    ...
    def create_analyse(self, uploaded_files, job_name):
        ...
        for content, path in self.get_files(uploaded_files):
            resum_result = self._ai.resume_cv(content, self.job)
            resum_schema = ResumFactory(self.job.get('id'), resum_result, path).create()
            score = self._ai.generate_score(resum_schema.content, self.job)
            analysis_schema = AnalysisFactory(
